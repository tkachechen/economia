...
Urbanpopulationpainel %>%
  ...
  mutate(
    Brasil = if_else(iso2c == "BR", "Brasil", 
                     if_else(iso2c == "CN", "China", 
                             if_else(iso2c == "US", "EUA", "Outros")))
  ) %>%
  ggplot(aes(x = year, y = SP.URB.TOTL, group = country, color = Brasil)) +
  geom_line(size = 0.8, alpha = 0.8) +
  scale_color_manual(values = c("Brasil" = "red", "China" = "green", "EUA" = "green", "Outros" = "black")) +
  labs(
...
# Identificar o país com o maior valor de crescimento urbano, excluindo 0 e NA
top_country <- Urbanpopulationtransversal %>%
  filter(!is.na(SP.URB.TOTL) & SP.URB.TOTL > 0) %>%
  arrange(desc(SP.URB.TOTL)) %>%
  slice(1) %>%
  pull(country)

# Exibir o nome do país com maior crescimento urbano
top_country
...
Urbanpopulationtransversal %>%
  ...
  scale_fill_manual(values = c("Brasil" = "red", "China" = "green", "EUA" = "green", "Outros" = "black")) +
  labs(
